...
class Misc extends CI_Controller
{
  ...
  public function check_session_status()
  {
    if (isset($_SESSION['admin_id']) && ($_SESSION['admin_id'] !== false) && ($_SESSION['admin_id'] !== "")) {
      ...
      $this->genmod->update_last_seen_time($_SESSION['admin_id'], 'admin_users');
    } else {
      $json['status'] = 0;
    }
  ...
  public function dldb()
  {
    ...
    $file_path = BASEPATH . "sqlite/inventory.sqlite"; //link to db file
  ...
  public function importdb()
  {
    ...
    if (file_exists(BASEPATH . "sqlite/inventory.sqlite")) {
      copy(BASEPATH . "sqlite/inventory.sqlite", BASEPATH . "sqlite/backups/" . time() . ".sqlite");
    }

    $config['upload_path'] = BASEPATH . "sqlite/"; //db files are stored in the basepath
    $config['allowed_types'] = 'sqlite';
    $config['file_ext_tolower'] = TRUE;
    $config['file_name'] = "inventory.sqlite";
